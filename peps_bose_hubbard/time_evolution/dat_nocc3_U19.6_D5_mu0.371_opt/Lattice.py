...
NNN_Tensor = np.zeros((N_UNIT,4),dtype=int)
FAR_X_Tensor = np.zeros((N_UNIT,10),dtype=int)
FAR_Y_Tensor = np.zeros((N_UNIT,10),dtype=int)
...
for i in range(N_UNIT):
    ...
    NNN_Tensor[i,0] = Tensor_list[(ix-1+LX)%LX,(iy+1)%LY] ## (-,+)
    NNN_Tensor[i,1] = Tensor_list[(ix+1)%LX,(iy+1)%LY] ## (++)
    NNN_Tensor[i,2] = Tensor_list[(ix+1)%LX,(iy-1+LY)%LY] ## (+,-)
    NNN_Tensor[i,3] = Tensor_list[(ix-1+LX)%LX,(iy-1+LY)%LY] ## (-,-)

    for j in range(10):
        FAR_X_Tensor[i,j] = Tensor_list[(ix+j)%LX,iy]
        FAR_Y_Tensor[i,j] = Tensor_list[ix,(iy+j)%LY]
