...
#    print(op_adag)
    ...
    val_nn = np.zeros((N_UNIT,2))
    val_nn11 = np.zeros((N_UNIT,4))
    val_nn_x_2 = np.zeros(N_UNIT)
    val_nn_x_3 = np.zeros(N_UNIT)
    val_nn_y_2 = np.zeros(N_UNIT)
    val_nn_y_3 = np.zeros(N_UNIT)
    ...
    for num in range(0,N_UNIT):
        ...
        val_nn[num,0] = np.real(Contract_two_sites_holizontal(C1[num],C2[num_j],C3[num_j],C4[num],eTt[num],eTt[num_j],eTr[num_j],eTb[num_j],eTb[num],eTl[num],Tn[num],Tn[num_j],op_n,op_n)/norm_x)
        ...
        val_nn[num,1] = np.real(Contract_two_sites_vertical(C1[num],C2[num],C3[num_j],C4[num_j],eTt[num],eTr[num],eTr[num_j],eTb[num_j],eTl[num_j],eTl[num],Tn[num],Tn[num_j],op_n,op_n)/norm_y)
        ...
        print "## t,U,mu,z,nocc,num,norm_x,norm_y,val_nn[num]",part,parU,parmu,parz,parnocc,num,norm_x,norm_y,val_nn[num]
    ...
    for i in range(0,N_UNIT):
        ...
        val_nn11[i,0] = np.real(Contract_four_sites(C1[k],C2[l],C3[i],C4[j],eTt[k],eTt[l],eTr[l],eTr[i],eTb[i],eTb[j],eTl[j],eTl[k],Tn[k],Tn[l],Tn[i],Tn[j],op_n,op_identity,op_n,op_identity)/norm)
        ##
        ## direction 1
        ##
        ## j--k
        ...
        val_nn11[i,1] = np.real(Contract_four_sites(C1[j],C2[k],C3[l],C4[i],eTt[j],eTt[k],eTr[k],eTr[l],eTb[l],eTb[i],eTl[i],eTl[j],Tn[j],Tn[k],Tn[l],Tn[i],op_identity,op_n,op_identity,op_n)/norm)
        ##
        ## direction 2
        ##
        ## i--j
        ...
        val_nn11[i,2] = np.real(Contract_four_sites(C1[i],C2[j],C3[k],C4[l],eTt[i],eTt[j],eTr[j],eTr[k],eTb[k],eTb[l],eTl[l],eTl[i],Tn[i],Tn[j],Tn[k],Tn[l],op_n,op_identity,op_n,op_identity)/norm)
        ##
        ## direction 3
        ##
        ## l--i
        ...
        val_nn11[i,3] = np.real(Contract_four_sites(C1[l],C2[i],C3[j],C4[k],eTt[l],eTt[i],eTr[i],eTr[j],eTb[j],eTb[k],eTl[k],eTl[l],Tn[l],Tn[i],Tn[j],Tn[k],op_identity,op_n,op_identity,op_n)/norm)
    for num in range(0,N_UNIT):
        for nnn in range(4):
    ...
    for num in range(0,N_UNIT):
        for nnn in range(4):
            print "## t,U,mu,z,nocc,num,nnn,norm,val_nn11[num,nnn]",part,parU,parmu,parz,parnocc,num,nnn,norm,val_nn11[num,nnn]
    ...
    for i in range(0,N_UNIT):
        ...
        val_aadag_x_2[i],val_adaga_x_2[i]
        #
        val_nn_x_2[i] = np.real(\
        Contract_scalar_3x1(\
         C1[i],eTt[i],eTt[j],eTt[k], C2[k],\
        eTl[i], Tn[i], Tn[j], Tn[k],eTr[k],\
         C4[i],eTb[i],eTb[j],eTb[k], C3[k],\
        op_n,op_identity,op_n\
        )\
        /norm)
        print "## t,U,mu,z,nocc,num,distx,disty,norm,val_nn",\
        part,parU,parmu,parz,parnocc,i,2,0,norm,\
        val_nn_x_2[i]
        #
        # x=3
        ...
        val_aadag_x_3[i],val_adaga_x_3[i]
        #
        val_nn_x_3[i] = np.real(\
        Contract_scalar_4x1(\
         C1[i],eTt[i],eTt[j],eTt[k],eTt[l], C2[l],\
        eTl[i], Tn[i], Tn[j], Tn[k], Tn[l],eTr[l],\
         C4[i],eTb[i],eTb[j],eTb[k],eTb[l], C3[l],\
        op_n,op_identity,op_identity,op_n\
        )\
        /norm)
        print "## t,U,mu,z,nocc,num,distx,disty,norm,val_nn",\
        part,parU,parmu,parz,parnocc,i,3,0,norm,\
        ...
        val_aadag_y_2[i],val_adaga_y_2[i]
        #
        val_nn_y_2[i] = np.real(\
        Contract_scalar_1x3(\
         C1[k],eTt[k], C2[k],\
        eTl[k], Tn[k],eTr[k],\
        eTl[j], Tn[j],eTr[j],\
        ...
        val_aadag_y_3[i],val_adaga_y_3[i]
        #
        val_nn_y_3[i] = np.real(\
        Contract_scalar_1x4(\
         C1[l],eTt[l], C2[l],\
        eTl[l], Tn[l],eTr[l],\
        eTl[k], Tn[k],eTr[k],\
    ...
    ave_aadag_dist01 = (np.sum(val_aadag) + np.sum(val_adaga))/N_UNIT/2/2
    ave_aadag_dist02 = (np.sum(val_aadag_x_2) + np.sum(val_adaga_x_2) + np.sum(val_aadag_y_2) + np.sum(val_adaga_y_2))/N_UNIT/2/2
    ave_aadag_dist03 = (np.sum(val_aadag_x_3) + np.sum(val_adaga_x_3) + np.sum(val_aadag_y_3) + np.sum(val_adaga_y_3))/N_UNIT/2/2
    ave_aadag_dist11 = (np.sum(val_aadag11) + np.sum(val_adaga11))/N_UNIT/4/2
    print "## t,U,mu,z,nocc,\
ave_aadag_dist01,ave_aadag_dist02,ave_aadag_dist03,\
ave_aadag_dist11,ave_aadag_dist22,ave_aadag_dist33\
",\
        part,parU,parmu,parz,parnocc,\
        ave_aadag_dist01,ave_aadag_dist02,ave_aadag_dist03,\
        ave_aadag_dist11

    ave_nn_dist01 = (np.sum(val_nn))/N_UNIT/2
    ave_nn_dist02 = (np.sum(val_nn_x_2) + np.sum(val_nn_y_2))/N_UNIT/2
    ave_nn_dist03 = (np.sum(val_nn_x_3) + np.sum(val_nn_y_3))/N_UNIT/2
    ave_nn_dist11 = (np.sum(val_nn11))/N_UNIT/4
    print "## t,U,mu,z,nocc,\
ave_nn_dist01,ave_nn_dist02,ave_nn_dist03,\
ave_nn_dist11,ave_nn_dist22,ave_nn_dist33\
",\
        part,parU,parmu,parz,parnocc,\
        ave_nn_dist01,ave_nn_dist02,ave_nn_dist03,\
        ave_nn_dist11
