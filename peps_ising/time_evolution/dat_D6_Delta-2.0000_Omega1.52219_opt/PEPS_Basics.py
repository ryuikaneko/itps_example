...
#tensor C1    e101  e11
#tensor C4    e74   e48
#tensor eT1   e11   e12   b11   bc11
#tensor eT7   e67   e74   b57   bc57
#tensor eT8   e48   e89   b58   bc58
#tensor eT9   e89   e910  b69   bc69
#tensor eT10  e910  e101  b110  bc110
#tensor Tn1   b110  b11   t12   t61   m1
#tensor Tn5   b58   t56   t45   b57   m5
#tensor Tn6   b69   t61   t63   t56   m6
#tensor Tn1c  bc110 bc11  tc12  tc61  m1c
#tensor Tn5c  bc58  tc56  tc45  bc57  m5c
#tensor Tn6c  bc69  tc61  tc63  tc56  m6c
#tensor op1 m1 m1c
#tensor op5 m5 m5c
#tensor op6 m6 m6c
#
#bond_dim 100 e11 e12 e67 e74 e48 e89 e910 e101
#bond_dim 10 t12 t45 t56 t61 t63
#bond_dim 10 tc12 tc45 tc56 tc61 tc63
#bond_dim 10 b11 b57 b58 b69 b110
#bond_dim 10 bc11 bc57 bc58 bc69 bc110
#bond_dim 2 m1 m5 m6
#bond_dim 2 m1c m5c m6c
##############################
# ((Tn1*((Tn1c*op1)*(eT1*(C1*eT10))))*(Tn6*((Tn6c*op6)*(eT9*(Tn5*((Tn5c*op5)*(eT7*(C4*eT8))))))))
# cpu_cost= 1.051e+14  memory= 3.01e+10
# final_bond_order (t12, tc12, e12, t63, tc63, t45, tc45, e67)
##############################
    LT = np.tensordot(
        np.tensordot(
            Tn1, np.tensordot(
...
#tensor C2    e22   e23
#tensor C3    e53   e36
#tensor eT2   e12   e22   b22   bc22
#tensor eT3   e23   e34   b23   bc23
#tensor eT4   e34   e45   b34   bc34
#tensor eT5   e45   e53   b45   bc45
#tensor eT6   e36   e67   b46   bc46
#tensor Tn2   t12   b22   b23   t23   m2
#tensor Tn3   t63   t23   b34   t34   m3
#tensor Tn4   t45   t34   b45   b46   m4
#tensor Tn2c  tc12  bc22  bc23  tc23  m2c
#tensor Tn3c  tc63  tc23  bc34  tc34  m3c
#tensor Tn4c  tc45  tc34  bc45  bc46  m4c
#tensor op2 m2 m2c
#tensor op3 m3 m3c
#tensor op4 m4 m4c
#
#bond_dim 100 e12 e22 e23 e34 e45 e53 e36 e67
#bond_dim 10 t12 t23 t34 t45 t63
#bond_dim 10 tc12 tc23 tc34 tc45 tc63
#bond_dim 10 b22 b23 b34 b45 b46
#bond_dim 10 bc22 bc23 bc34 bc45 bc46
#bond_dim 2 m2 m3 m4
#bond_dim 2 m2c m3c m4c
##############################
# ((Tn2*((Tn2c*op2)*(eT2*(C2*eT3))))*(Tn3*((Tn3c*op3)*(eT4*(Tn4*((Tn4c*op4)*(eT5*(C3*eT6))))))))
# cpu_cost= 1.051e+14  memory= 3.01e+10
# final_bond_order (t12, tc12, e12, t63, tc63, t45, tc45, e67)
##############################
    RT = np.tensordot(
...
##############################
    return np.tensordot(LT,RT,([0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7]))
...
## for simple update
def Simple_update_bond(Tn1,Tn2,lambda1,lambda2,op12,connect1):
    ## --前処理--
    if connect1==0:
        connect2 = 2
    elif connect1 ==1:
...
#        U,s,VT = linalg.svd(Theta.transpose(0,2,1,3).reshape(R1.shape[0]*m1,R2.shape[0]*m2),full_matrices=False) 
    ...
    ## truncation
#
    print "### lambda",s[0],s[1],s[2],s[3],s[4],s[5],s[6],s[7],s[8],s[9],s[10],s[11],s[12],s[13],s[14],s[15]
    ss = s/s[0]
    print "### normalized lambda",ss[0],ss[1],ss[2],ss[3],ss[4],ss[5],ss[6],ss[7],ss[8],ss[9],ss[10],ss[11],ss[12],ss[13],ss[14],ss[15]
#
    lambda_c = s[0:dc]
    Uc = U[:,0:dc]
    VTc = VT[0:dc,:]
    ...
    if connect1==0:
        Tn1_new = np.tensordot(Q1,Uc.reshape(R1.shape[0],m1,dc),(0,0)).transpose(4,0,1,2,3)
    elif connect1 == 1:
        Tn1_new = np.tensordot(Q1,Uc.reshape(R1.shape[0],m1,dc),(0,0)).transpose(0,4,1,2,3)
    elif connect1 == 2:
        Tn1_new = np.tensordot(Q1,Uc.reshape(R1.shape[0],m1,dc),(0,0)).transpose(0,1,4,2,3)
    else:
        Tn1_new = np.tensordot(Q1,Uc.reshape(R1.shape[0],m1,dc),(0,0)).transpose(0,1,2,4,3)
        
    if connect2 == 0:
        Tn2_new = np.tensordot(Q2,VTc.reshape(dc,R2.shape[0],m2),(0,1)).transpose(3,0,1,2,4)
    elif connect2 == 1:
        Tn2_new = np.tensordot(Q2,VTc.reshape(dc,R2.shape[0],m2),(0,1)).transpose(0,3,1,2,4)
    elif connect2 == 2:
        Tn2_new = np.tensordot(Q2,VTc.reshape(dc,R2.shape[0],m2),(0,1)).transpose(0,1,3,2,4)
    elif connect2 == 3:
        Tn2_new = np.tensordot(Q2,VTc.reshape(dc,R2.shape[0],m2),(0,1)).transpose(0,1,2,3,4)    
